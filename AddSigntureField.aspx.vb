...
Partial Class AddSigntureField
    ...
    Protected Sub cpSigndoc_Callback(source As Object, e As DevExpress.Web.CallbackEventArgs)
        ...
        Dim code As String() = arr(1).Split(",")
        Dim tk As String() = arr(2).Split(",")
        Dim info As String() = arr(3).Split(",")
        Dim idfile As Integer = Session("idFile")
        Dim ttk As String() = arr(4).Split(",")
        Dim htk As String() = arr(5).Split(",")
        Dim trangky As String() = arr(6).Split(",")
        Dim phuongthuc As String() = arr(7).Split(",")
